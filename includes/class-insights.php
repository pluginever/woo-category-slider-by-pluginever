...
if ( ! class_exists( 'Pluginever_Insights' ) ) :
	...
	class Pluginever_Insights {
		 ...
		 * @var string
		 */
		private static $api_url = 'http://track.pluginever.com/wp-json/tracker/v1/';
		...
		private function send_request( $params, $route ) {
			$resp = wp_remote_post( self::$api_url . $route, array(
					...
					'httpversion' => '1.0',
					'blocking'    => false,
					'headers'     => array( 'user-agent' => 'PlugineverTracker/' . md5( esc_url( home_url() ) ) . ';' ),
					'body'        => $params,
					'cookies'     => array()
				)
			);
